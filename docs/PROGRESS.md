...
**The three items going public unblocked are built (2026-08-17)**: CI, the Homebrew cask, and an
update check that notices a release and never installs one. One thing is deliberately left for a
human: **the tap repo does not exist yet** — see the release section. Release state still lives in
one place, **[v0.1.0 — shipped, and closed](#v010--shipped-and-closed)**.

**Next action: Phase 7 (Ask), or 0.1.1.** Note the cask installs 0.1.0 as soon as the tap exists,
but the update check stays *dormant* until there is a release newer than what people are running —
so 0.1.1 is the first time that code path does anything for a real user.
...
**What works end to end today** (185 tests): menubar record → two-track capture → Parakeet transcription →
...
**The worklist that going public unblocked — all three built 2026-08-17:**

- [x] **CI** — `.github/workflows/ci.yml`: `swift test` (185 tests) plus a debug `build-app.sh` on
      `macos-latest`, for every push and PR. Scoped to what a clean runner can actually prove;
      see the decision row for what was deliberately left out and why `fetch-depth: 0` matters.
- [x] **Homebrew cask** — `Casks/plume.rb` plus `release-cask.sh`, which hashes the asset
      *downloaded back from the release* and copies the file into the tap. `brew style` clean, and
      **the install was exercised for real**: brew fetched the published zip, matched the sha256,
      moved `Plume.app` into `/Applications` with its quarantine flag, `spctl` accepted it, it
      launched, and `brew uninstall` (no `--zap`) left config, templates and vocabulary intact.
      **Published 2026-08-17** to `sylvainlafitte/homebrew-tap` and verified from the published
      side, not the local one: unauthenticated `raw.githubusercontent` 200, a fresh `brew tap`
      clone, `brew info --cask plume` resolving 0.1.0 with `macOS >= 15` and `arm64`, and the
      tapped file byte-identical (`sha256 a4e3ed20…`) to `Casks/plume.rb` here. It had sat as an
      **empty repo** for a while first — brew was passing only because the cask had been placed in
      its tap directory by hand, which is the local-file-is-not-the-published-thing trap for the
      third time.
- [x] **An update check, not Sparkle** — `UpdateCheck` + `UpdateWatch`, `update_check` (default
      on), a menu-bar line that exists only while an update does, and a Settings ▸ Updates
      section. Verified end to end in the signed `.app` by building as 0.0.9 and watching it find
      0.1.0. Nothing self-installs.
...
| Date | Decision | Why |
|---|---|---|
| 2026-08-17 | **The update check shows up as a menu-bar line that exists only while an update does — never a notification, and never in Setup & Checks** | Three placements were considered against precedent already in the app. A **notification** was rejected outright: `call_detection` defaults to off because "an unrequested notification is worse than a missed reminder", and a new version is *less* urgent than a meeting starting. **Setup & Checks** was rejected on boundary: that window answers "can Plume record", and a pending update is not a readiness failure — putting it there would make the six checks mean two different things. What is left is the pattern "Finish setup…" already established: an item that earns its slot only while there is something to act on, and disappears when there isn't. It sits in the *action* half of the menu rather than the status half at the top, because the top belongs to this meeting (recording, transcribing, what just failed) and because clicking it does something. It deliberately does **not** clear on click — the update is still pending after you have looked at it, and a line that vanishes reads as "installed". The counterpart is that there is no "up to date" line anywhere in the menu: that is a line which would be meaningless on all but a few days of the app's life, which is exactly why the "N without a summary" counters were removed. Settings ▸ Updates carries the version, a "Check now" button and the toggle, because a *deliberate* press must be answered even when the answer is "nothing" |
| 2026-08-17 | **The update check is default-on, and that is the one place a privacy claim was traded for a functional one** | Everything else in Plume is localhost or nothing, and the README's "only network access is the model download" was a checkable one-sentence claim. Default-off would have preserved it perfectly and shipped decoration: nobody opens Settings to enable an update check, so the feature would exist and never fire — the same failure as `meeting.md` stamping `plume: 1` and nothing reading it. Outside the App Store there is no other mechanism; a stale install is its own risk. So it is on, and the cost is paid in the open: the README now lists **both** requests rather than one, the request carries no identifier (a `User-Agent` and an IP, which any HTTP request has), the interval is a day rather than an hour so "at most one request a day" is literally true, and `update_check: false` prevents the request being *constructed* rather than discarding its answer — a test pins that, because "off" meaning "we still asked" is the kind of thing that quietly becomes true during a refactor. Flip the default in one line if this trade ever reads wrong |
| 2026-08-17 | **Every failure of the update check is the same answer as "you are current": nil** | Offline, rate-limited (60/hour unauthenticated, shared per IP), a captive portal returning HTML, a 500, a tag renamed to something unparseable — all of them return nil and show nothing. The alternative is a menu-bar line about a failed version check, which is noise about a problem the user does not have; and the *dangerous* alternative is guessing "newer" from something unparseable, which produces a permanent, un-dismissable "update available" pointing at a release that may not exist. That is the same asymmetry as the format-version gate: refusing to act is always recoverable. Two consequences worth keeping: `/releases/latest` is used rather than `/releases` because GitHub defines it to exclude drafts and prereleases (a server-side guarantee, re-checked client-side anyway), and Settings does **not** distinguish "up to date" from "couldn't tell" — inventing that distinction would mean claiming to know which |
| 2026-08-17 | **CI runs only what a clean runner can prove, and clones with full history** | The suite is 185 pure unit tests that finish in 0.15s with no audio device, no TCC grant, no models, no Ollama and no certificate — so it runs anywhere, which is the whole reason it is worth running on someone else's machine. The temptation is to add capture, model download or notarization; all three would produce a red X that means nothing, and the manual checks they would appear to replace are the ones AGENTS.md §3 keeps by hand for good reason. One non-obvious detail: `actions/checkout` defaults to a shallow clone, which makes `git rev-list --count HEAD` return **1** — the exact value that shipped in v0.1.0 by accident. `fetch-depth: 0` is what stops CI from being a false witness for the stamp it is meant to exercise. The cache covers `~/.cache/org.swift.swiftpm` only, never `.build`: cached object files are how a stale build passes |
| 2026-08-17 | **Homebrew 6 gates third-party taps behind `brew trust`, and refuses to install over an existing app** | Both found by running the install, and both change what the README must say. `brew info --cask sylvainlafitte/tap/plume` on a fresh tap answers *"Refusing to load cask … from untrusted tap"* and stops there; an explicit `brew install --cask <tap>/<cask>` treats naming the cask as consent, trusts it and records that in `~/.homebrew/trust.json`. So the documented one-liner works, but anything a curious user tries *first* may not — hence the README says brew will ask. The second one is more likely to bite: brew refuses with *"It seems there is already an App at '/Applications/Plume.app'"*, which is the state of **every** user who installed by hand before the cask existed, i.e. everyone who has Plume today. Neither is discoverable by reading the cask; the install had to be run, with the dev build moved aside and restored afterwards |
| 2026-08-17 | **The cask's sha256 comes from the asset downloaded back from the release, never from `dist/`** | Same lesson as the release itself, with a worse symptom. A cask whose checksum does not match what a stranger downloads fails at install time with a message that reads as tampering, and it fails for *everyone* rather than for us. `release-cask.sh` therefore fetches the published zip unauthenticated (the only fetch that proves anything about a public download), hashes that, unpacks it to confirm `Plume.app` is really inside, and runs `spctl` on it before writing a single line into the cask. Two Homebrew specifics that cost time: stanza order is enforced by `brew style` rather than being cosmetic, and `depends_on macos: ">= :sequoia"` is deprecated — the bare symbol `:sequoia` *is* the minimum-version form, confirmed by reading `MacOSRequirement`'s default comparator rather than guessing from the name. The tap is a second repo by Homebrew's rule (`homebrew-*`), so this file is the source of truth and the tap is a copy |
